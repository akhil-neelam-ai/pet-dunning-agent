...
def metric_card(label: str, value: str, delta: str = None, help_text: str = None):
    """
    Render a metric card using Streamlit's native components
    """
    # Build the HTML parts separately
    delta_html = ""
    if delta:
        delta_html = f'<div style="color: #E8F5E9; font-size: 14px; margin-top: 4px;">{delta}</div>'

    # Create a container with custom styling - removed help_text to eliminate black boxes
    st.markdown(
        f"""
